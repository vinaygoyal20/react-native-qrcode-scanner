...
export default class QRCodeScanner extends Component {
  ...
  constructor(props) {
    ...
    this.cameraAttrs = {};
    if (Platform.OS === 'ios') {
      this.cameraAttrs.onBarCodeRead = this._handleBarCodeRead;
    } else {
      this.cameraAttrs.onGoogleVisionBarcodesDetected = ({barcodes}) => {
        let barcodeData = barcodes[0];
        if (barcodeData.type === Camera.Constants.BarCodeType.qr) {
          this._handleBarCodeRead(barcodeData);
        }
      };
    }
  }
